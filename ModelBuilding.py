...
class Predictor:
    ...
    def prepare_data(self, split_data, train_test):
        ...
        data.loc[:, ~cats].join(pd.DataFrame(data=onehotencoder.fit_transform(data.loc[:, cats]).toarray(),
                                             columns=onehotencoder.get_feature_names()))
    ...
    def set_classifier_properties(self):
        ...
        if self.type == "Regression":
            self.chosen_classifier = st.sidebar.selectbox("Please choose a classifier",
    ...
    def set_features(self):
        self.features = st.multiselect('Please choose the features including target variable that go into the model',
                                       self.data.columns)
